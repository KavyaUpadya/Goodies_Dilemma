...
a = int(a)
file=open("Execution_input.txt",'r') #the inputs given in the file are read
fout=open("Execution_output.txt","a") # new ouput file is created where the obtained output is written 
line=[]
da=[]
lit=[]
str1=dict()
for data in file:
    ...
    if da[1] !='':
        lit.append(int(da[1]))
        str1[lit[-1]]=da[0]
lit.sort() #sorts the goodies based on price in the input file
temp=[]
diff=111111111
sum=0
